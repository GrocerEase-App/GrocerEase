...
final class TraderJoesScraper: NSObject, Scraper {
        ...
        -> [GroceryItem]
    {
        ...
        return items.map { i, doc in
            let item = GroceryItem(
                name: doc["item_title"].stringValue,
                store: store
            )
            ...
            item.price =
                doc["price_range"]["minimum_price"]["final_price"]["value"]
                .doubleValue
            ...
            item.url = URL(
                string:
                    "https://www.traderjoes.com/\(doc["url_key"].stringValue)"
            )
            item.imageUrl = URL(
                string:
                    "https://www.traderjoes.com\(doc["primary_image"].stringValue)"
            )
            item.searchRank = i
            return item
        }
    ...
    func findStores(
        ...
        let base = URL(
            string: "https://alphaapi.brandify.com/rest/locatorsearch"
        )!
            ...
            return stores.map { store in
                ...
                return GroceryStore(
                    ...
                    location: CLLocationCoordinate2D(
                        latitude: Double(store["latitude"].stringValue)!,
                        longitude: Double(store["longitude"].stringValue)!
                    ),
                    address: address,
                    source: .traderjoes
                )
