...
final class TargetScraper: NSObject, Scraper {
        ...
        webView.evaluateJavaScript(js) { result, error in
            ...
            guard let htmlString = result as? String else {
                self.setupContinuation?.resume(
                    throwing: "Couldn't parse Target homepage html"
                )
                return
            }
            ...
            let keys = matches.compactMap {
                Range($0.range(at: 1), in: htmlString).map {
                    String(htmlString[$0])
                }
            }
    ...
    func findStores(
        ...
        guard
            let visitorCookie = await webView.getAllCookiesAsync().first(
                where: {
                    $0.name == "visitorId" && $0.domain.contains("target.com")
                })
        else {
            throw "No visitor_id found."
        }

        guard let zip = await location.fetchZipCode() else {
        ...
        let stores = apiResponse["data"]["nearby_stores"]["stores"].arrayValue
            .map { store in
                // ---------- Build full address ----------
                ...
                if let line1 = store["mailing_address"]["address_line1"].string,
                    ...
                    let country = store["mailing_address"]["country_code"]
                        .string,
        ...
        -> [GroceryItem]
    {
        ...
        guard let zip = await store.list?.location?.fetchZipCode() else {
            throw "Zip Code required for Target Scraper"
        }
        ...
        components?.queryItems = [
            ...
            URLQueryItem(
                name: "scheduled_delivery_store_id",
        ...
        let items = products.map { i, product in
            ...
            // --- SNAP eligibility --------------------------------------------------
            newItem.snap =
                product["item"]["compliance"]["is_snap_eligible"].boolValue
            ...
            // --- Image -------------------------------------------------------------
            if let urlString = product["item"]["enrichment"]["images"][
                "primary_image_url"
            ].string,
            ...
            if let deptId = product["item"]["merchandise_classification"][
                "department_id"
            ].int {
